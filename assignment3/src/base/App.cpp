...
void App::renderSkeleton() {
	...
	for (auto i = 0u; i < transforms.size(); ++i) {
		...
		Vec4f pos_aux = transforms[i] * Vec4f(0, 0, 0, 1);
		joint_world_pos = Vec3f(pos_aux[0], pos_aux[1], pos_aux[2]);
		...
		Mat3f transforms_aux = Mat3f();
		transforms_aux.setCol(0, Vec3f(transforms[i].m00, transforms[i].m01, transforms[i].m02));
		transforms_aux.setCol(1, Vec3f(transforms[i].m10, transforms[i].m11, transforms[i].m12));
		transforms_aux.setCol(2, Vec3f(transforms[i].m20, transforms[i].m21, transforms[i].m22));

		// draw the x axis... ("right")
		glColor3f(1, 0, 0); // red
		// glVertex3f(...); glVertex3f(...);

		right = scale * (transforms_aux.transposed() * Vec3f(1, 0, 0));
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);  glVertex3f(joint_world_pos.x + right[0], joint_world_pos.y + right[1], joint_world_pos.z + right[2]);
		...
		up = scale * (transforms_aux.transposed() * Vec3f(0, 1, 0));
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);  glVertex3f(joint_world_pos.x + up[0], joint_world_pos.y + up[1], joint_world_pos.z + up[2]);
		...
		ahead = scale * (transforms_aux.transposed() * Vec3f(0, 0, 1));
		glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z);  glVertex3f(joint_world_pos.x + ahead[0], joint_world_pos.y + ahead[1], joint_world_pos.z + ahead[2]);
		...
		if (skel_.getJointParent(i) != -1) {
			Vec3f joint_parent_world_pos;
			Vec4f pos_aux2 = transforms[skel_.getJointParent(i)] * Vec4f(0, 0, 0, 1);
			joint_parent_world_pos = Vec3f(pos_aux2[0], pos_aux2[1], pos_aux2[2]);

			glColor3f(1, 1, 1);
			glVertex3f(joint_world_pos.x, joint_world_pos.y, joint_world_pos.z); glVertex3f(joint_parent_world_pos.x, joint_parent_world_pos.y, joint_parent_world_pos.z);
		}
...
vector<Vertex> App::computeSSD(const vector<WeightedVertex>& source_vertices) {
	...
	for (const auto& sv : source_vertices) {
		...
		for (int j = 0; j < std::size(sv.joints); j++) {
			v.position += (sv.weights[j] * ssd_transforms[sv.joints[j]] * Vec4f(sv.position,1)).getXYZ();
			v.normal += (((sv.weights[j] * ssd_transforms[sv.joints[j]]).inverted().transposed()) * Vec4f(sv.normal, 1)).getXYZ();
		}
