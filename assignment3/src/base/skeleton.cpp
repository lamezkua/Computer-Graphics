...
void Skeleton::setJointRotation(unsigned index, Vec3f euler_angles) {
	...
	FW::Mat3f x_rot = FW::Mat3f::rotation(FW::Vec3f(1, 0, 0), euler_angles[0]);
	FW::Mat3f y_rot = FW::Mat3f::rotation(FW::Vec3f(0, 1, 0), euler_angles[1]);
	FW::Mat3f z_rot = FW::Mat3f::rotation(FW::Vec3f(0, 0, 1), euler_angles[2]);
	FW::Mat3f xyz_rot = x_rot * y_rot * z_rot;
	joint.to_parent.m00 = xyz_rot.m00;
	joint.to_parent.m01 = xyz_rot.m01;
	joint.to_parent.m02 = xyz_rot.m02;
	joint.to_parent.m10 = xyz_rot.m10;
	joint.to_parent.m11 = xyz_rot.m11;
	joint.to_parent.m11 = xyz_rot.m11;
	joint.to_parent.m20 = xyz_rot.m20;
	joint.to_parent.m21 = xyz_rot.m21;
	joint.to_parent.m22 = xyz_rot.m22;
}
...
void Skeleton::updateToWorldTransforms(unsigned joint_index, const Mat4f& parent_to_world) {
	...
	FW::Mat4f newS = parent_to_world * joint.to_parent;
	joints_[joint_index].to_world = newS;
	// recursive call to children, using new transformation
	for (int i = 0; i < joint.children.size(); ++i) {
		updateToWorldTransforms(joint.children[i], newS);
	}
}
...
void Skeleton::computeToBindTransforms() {
	...
	FW::Mat4f bind_transformation;
	for (int j = 0; j < joints_.size(); j++) {
		bind_transformation = joints_[j].to_world.inverted();
		joints_[j].to_bind_joint = bind_transformation;
	}
}
...
vector<Mat4f> Skeleton::getSSDTransforms() {
	...
	for (int j = 0; j < joints_.size(); j++) {
		transforms.push_back(joints_[j].to_world * joints_[j].to_bind_joint);
	}
