...
vector<Vertex> loadUserGeneratedModel() {
	...
	for(auto i = 0u; i < faces; ++i)	{
		...
		Vec3f normal;
		v0.position = Vec3f(FW::cos(angle_increment * i) * radius, 0 - height,FW::sin(angle_increment * i) * radius);
		v1.position = Vec3f(FW::cos(angle_increment * (i + 1)) * radius, 0 - height, FW::sin(angle_increment * (i + 1)) * radius);
		v2.position = Vec3f(0, 0, 0);
		normal = cross(v0.position, v1.position);
...
bool App::handleEvent(const Window::Event& ev) {
	...
	if (seconds_passed > 0.0)
		camera_rotation_angle_ += 0.005 * FW_PI;
...
void App::initRendering() {
	...
	auto shader_program = new GLContext::Program(
		...
		void main()
		{
			...
			vec3 corr_normal = (transpose(inverse(uModelToWorld)) * vec4(aNormal, 1.0)).xyz;
			float clampedCosine = clamp(dot(corr_normal, directionToLight), 0.0, 1.0);
...
void App::render() {
	...
	Vec2i windowsize= window_.getSize();
	int viewportx, viewporty, sidew;
	viewportx = windowsize.x;
	viewporty = windowsize.y;
	if (viewportx < viewporty)
		sidew = viewportx;
	else
		sidew = viewporty;
	glViewport(windowsize.x/2-sidew/2,windowsize.y/2-sidew/2,sidew, sidew);
	...
	// modelToWorld.setCol(3, Vec4f(current_translation.x, current_translation.y, current_translation.z, 1));
	
	Mat4f translation, rotation, scalation, modelToWorld_temp, modelToWorld;

	scalation.setCol(0, Vec4f(current_scaling.x, 0, 0, 0));
	scalation.setCol(1, Vec4f(0, 1, 0, 0));
	scalation.setCol(2, Vec4f(0, 0, 1, 0));
	scalation.setCol(3, Vec4f(0, 0, 0, 1));

	rotation.setCol(0, Vec4f(cos(object_rotation_angle_), 0, -sin(object_rotation_angle_), 0));
	rotation.setCol(1, Vec4f(0, 1, 0, 0));
	rotation.setCol(2, Vec4f(sin(object_rotation_angle_), 0, cos(object_rotation_angle_), 0));
	rotation.setCol(3, Vec4f(0, 0, 0, 1));

	modelToWorld_temp = rotation * scalation;

	translation.setCol(0, Vec4f(1, 0, 0, 0));
	translation.setCol(1, Vec4f(0, 1, 0, 0));
	translation.setCol(2, Vec4f(0, 0, 1, 0));
	translation.setCol(3, Vec4f(current_translation.x, current_translation.y, current_translation.z, 1));

	modelToWorld = translation * modelToWorld_temp;
...
vector<Vertex> App::loadObjFileModel(string filename) {
	...
	while(getline(input, line)) {
		...
		if (s == "v") { // vertex position
			...
			// cout << face[0] << " " << face[1] << " " << face[2] << " " << face[3] << " " << face[4] << " " << face[5] << endl;
		}
	}
