...
void App::renderWithNormalMap(GLContext* gl, Mat4f worldToCamera, Mat4f projection)
{
	...
	{
		lightDir[i] = (worldToCamera * Vec4f( lightDir[i].normalized(), 0.0f )).getXYZ(); //4.1 .1 (Don't consider the whole for loop if button is pressed)
	}
