...
GLuint render(RayTracer& ray_tracer, SceneParser& scene, const Args& args) {
	...
	for (int j = 0; j < args.height; ++j) {
		...
		for (int i = 0; i < args.width; ++i) {
			...
			Vec3f sample_color_avg{ 0.0f,0.0f,0.0f };
			for (int n = 0; n < args.num_samples; ++n) {
				...
				if (args.display_uv)
					sample_color = Vec3f(float(i)/ args.width, float(j)/ args.height, 1.0f);
				
				image->setVec4f(Vec2i(i,j), Vec4f(sample_color_avg/(float(n) + 1.0f), 1.0f));
				...
				if (depth_image) {
					...
					if (hit.t >= args.depth_min && hit.t <= args.depth_max) {
						f = (((args.depth_min - hit.t) / (args.depth_max - args.depth_min)) + 1.0f);
					 }
