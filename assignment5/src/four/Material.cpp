		...
		bool shade_back) const
{
	...
	Vec3f aux_normal = hit.normal;
	if (shade_back) {
		aux_normal = -hit.normal;
	}
	
	float  d = 0;
	if (dir_to_light.dot(hit.normal) > 0) {
		d = dir_to_light.dot(hit.normal);
	}
	
	float s_aux = 0;
	if (-ray.direction.dot(-dir_to_light - 2.0f*(-dir_to_light.dot(aux_normal)) * aux_normal) > 0 && d > 0) {
		s_aux = -ray.direction.dot(-dir_to_light - 2.0f * (-dir_to_light.dot(aux_normal)) * aux_normal);
	}

	Vec3f answer = incident_intensity * d * this->diffuse_color_ + incident_intensity * specular_color_ * std::pow(s_aux,exponent_);
