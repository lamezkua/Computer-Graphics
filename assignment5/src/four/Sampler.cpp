...
Vec2f UniformSampler::getSamplePosition(int i) {
	...
	//return Vec2f(0, 0);
	return Vec2f(m_generator.getF32(), m_generator.getF32());
}
...
Vec2f RegularSampler::getSamplePosition(int n) {
	...
	return Vec2f((2.0f * (n % m_dim) + 1.0f) * (1.0f / (2.0f * m_dim)), (2.0f * (n / m_dim)  + 1.0f) * (1.0f / (2.0f * m_dim)));
}
...
Vec2f JitteredSampler::getSamplePosition(int n) {
	...
	//return Vec2f(0,0);
	return Vec2f((2.0f * (n % m_dim) + 1.0f) * (1.0f / (2.0f * m_dim)) + m_generator.getF32() * (1.0f / m_dim), (2.0f * (n / m_dim) + 1.0f) * (1.0f / (2.0f * m_dim)) + m_generator.getF32() * (1.0f / m_dim));
}
