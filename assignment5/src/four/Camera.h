...
public:
	...
	static inline FW::Vec2f normalizedImageCoordinateFromPixelCoordinate(const FW::Vec2f& pixel, const FW::Vec2i& imageSize) {
		...
		return FW::Vec2f((-1.0f + (((pixel.x - 0.0f) * (1.0f - (-1.0f))) / (imageSize.x - 0.0f))), -(-1.0f + (((pixel.y - 0.0f) * (1.0f - (-1.0f))) / (imageSize.y - 0.0f))));
	}
...
public:
	...
	virtual Ray generateRay(const FW::Vec2f& point) {
		...
		return Ray(FW::Vec3f(center + point.x * size/2.0f * horizontal + point.y * size/2.0f * up), FW::Vec3f(direction));
	}
...
public:
	...
	{	
		...
		//return Ray( FW::Vec3f(0.0f), FW::Vec3f(0.0f) );
		return Ray(this->center, (point.x * this->horizontal + 1.0f * point.y * this->up + (1.0f/FW::tan(this->fov_angle/2.0f)) * this->direction).normalized());
	}
