...
void PointLight::getIncidentIllumination(const Vec3f& p,
	...
	dir_to_light = (-p + this->position_).normalized();
	distance = (-p + this->position_).length();
	incident_intensity = intensity_ / (quadratic_attenuation_ * pow(distance,2)  + linear_attenuation_ * distance + constant_attenuation_);
